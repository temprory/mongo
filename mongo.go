...
func (m *Mongo) EnsureIndex(dbname string, cname string, keys []string) {
	...
	if err := session.DB(dbname).C(cname).EnsureIndex(index); err != nil {
		logInfo("mongodb %s.%s EnsureIndex Failed, Error: %v", dbname, cname, err)
	}
}

func NewMongo(conf MongoConf) *Mongo {
	logInfo("NewMongo Connect To Mongo ...")
	...
	if err != nil {
		logFatal("NewMongo mgo.DialWithTimeout failed: %v", err)
	}
	...
	logInfo("NewMongo(pool size: %d) Connect To Mongo Success", len(mongo.sessions))
